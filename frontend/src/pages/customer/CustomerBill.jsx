...
const CustomerBill = () => {
  const customerPlaceOrder = useCustomerPlaceOrder();
  ...
  //reqType = "placeOrder"
  const isPlacingOrder = customerPlaceOrder.loading;
  ...
  const placeOrder = async () => {
    ...
    try {
      const result = await customerPlaceOrder.customerPlaceOrder({
        restaurantName,
        data: { orders, customer, token },
      });
      toast.success("Order placed successfully.");
      navigate(`/customer/${restaurantName}/CustomerOrderDeteal`);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-100 p-3 max-w-md mx-auto flex flex-col justify-between    ">
      ...
      <div className="mb-8 sticky top-0 bg-gray-100 p-1 pb-3 pt-2">
        <div className="flex  gap-3 items-center   ">
          <HiChevronLeft
            onClick={goToCustomerHomepaje}
            className="text-green-600 text-4xl border-1 border-green-600 rounded-full"
          />
