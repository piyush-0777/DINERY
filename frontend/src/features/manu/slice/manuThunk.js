...
import { menuService } from "../service/manuService";
...
export const deletFoodThunk = createAsyncThunk(
  "deletFoodThunk",
  async (data, thunkAPI) => {
    try {
...
export const deletCategoryThunk = createAsyncThunk(
  "deletCategoryThunk",
  async (data, thunkAPI) => {
    try {
...
export const changeAvailablityThunk = createAsyncThunk(
  "changeAvailablity",
  async (id, thunkAPI) => {
    try {
      const res = await menuService.changeAvailablity(id);
      return res;
    } catch (error) {
      return thunkAPI.rejectWithValue({
        status: error.status || 500,
