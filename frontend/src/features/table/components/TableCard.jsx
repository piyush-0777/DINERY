...
import { useUpdateTableStatus, useDeleteTable } from "../index";
import { useUpdateOrderStatus } from "../../order";
...
const TableCard = ({ table, onOpen, onAddOrder, onShowQR, onBill }) => {
  ...
  const { loading, reqtype, error, success } = useSelector(
    (state) => state.orders,
  ); // this is for order loading
  ...
  const updatingTable = updateOrderStatus.loading

  const deletingTable = deleteTable.loading

  const updatingOrder = updateOrderStatus.loading
  ...
  const onChangeTableStatus = async (table, status) => {
    try {
      const result = await updateTableStatus.updateTableStatus({
        id: table?._id,
        status,
      });
  ...
  const onDelete = async (id) => {
    ...
    try {
      const result = await deleteTable.deleteTable(id);
  ...
  return (
    ...
    >
      ...
      {/* Status Dot */}
      <span
        className={`absolute top-3 right-3 w-3 h-3 rounded-full ${ui.dot}`}
      />
      ...
      {table.status === TABLE_STATUS.ACTIVE && (
        <div className="flex justify-between text-sm text-neutral-400">
          ...
          <span className="text-yellow-400 font-semibold">{elapsed}</span>
        </div>
      )}
  ...
  "
      >
        <div className="flex flex-col gap-3">
          ...
          {table.status === TABLE_STATUS.ACTIVE && (
            <SwitchBtn
              ...
              onClick={() => onChangeTableStatus(table, TABLE_STATUS.AVAILABLE)}
            />
          )}
          ...
          {table.status === TABLE_STATUS.OCCUPIED &&
            ...
            table?.order?.status !== "cancelled" && (
              <SwitchBtn
                ...
                onClick={() =>
                  onChangeOrderStatus(table?.order?._id, getNextOrderState())
                }
              />
            )}
        </div>
        ...
        <div className="flex justify-end gap-2">
          ...
          {table.status === TABLE_STATUS.ACTIVE && (
            <>
              <IconBtn onClick={() => onAddOrder(table)} color="bg-green-600">
                <Plus size={16} />
              </IconBtn>
...
const SwitchBtn = ({ label, onClick, loading = false }) => (
  <button
    disabled={loading}
    onClick={(e) => {
      e.stopPropagation();
