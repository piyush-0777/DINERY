...
import { TABLE_STATUS } from "../components/TableStatus";
import {
  addTableThunk,
  deleteTableThunk,
  updateTableThunk,
...
import { loadDashbordThunk } from "../../loadside";
import { cashPaymentThunk } from "../../bill";
import { updateOrderStatusThunk } from "../../order";
  ...
  extraReducers: (builter) => {
    builter
      ...
      .addCase(cashPaymentThunk.fulfilled, (state, action) => {
        state.tables = state.tables.map((e) => {
          if (e._id === action.payload.data.table._id)
